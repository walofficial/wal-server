...
if settings.env != "local":
    ...
    client = TranscoderServiceClient(
        client_options=client_options_lib.ClientOptions(
            api_endpoint=settings.mock_gcp_transcoder_endpoint,
        ),
        credentials=type(
            "MockCredentials",
...
def transcode_job(
    ...
    config_default = resources.JobConfig(**config)

    if topic_path:
        config_default.pubsub_destination = resources.PubsubDestination(
            topic=topic_path
        )
...
def create_transcode_job_template(
    ...
    location: str,
    template_id: str,
    job_config: resources.JobConfig,
...
def get_job_with_name(job_full_name: str) -> resources.Job:
...
async def wait_for_job_completion(job_name: str, poll_interval=3):
    while True:
        ...
        state = job.state
        if state == resources.Job.ProcessingState.SUCCEEDED:
            logger.info("to mp4 job has been completed")
            break
        elif state == resources.Job.ProcessingState.FAILED:
