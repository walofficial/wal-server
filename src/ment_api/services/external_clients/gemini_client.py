...
"""
    ...
    async def generate_image_description(
        self, image_url: str, location_name: str = None
    ) -> Optional[str]:
        """
        Generate a descriptive caption for a location image using Gemini.
        Enhanced with comprehensive Langfuse observability.
        ...
        Returns:
            String with a description of the image or None if generation failed
        """
        logger.info(
            "Starting Gemini image description generation",
            extra={
                "json_fields": {
                    "image_url": image_url,
                    "has_location_name": location_name is not None,
                    "operation": "gemini_image_description_start",
                },
                "labels": {
                    "component": "gemini_client",
                    "phase": "image_description",
                },
            },
        )
        ...
        self.langfuse.update_current_trace(
            ...
            metadata={"operation": "generate_image_description"},
        )
        ...
        if not image:
            ...
            logger.error(
                "Image description generation failed - download error",
                extra={
                    "json_fields": {
                        "image_url": image_url,
                        "error": error_msg,
                        "operation": "gemini_image_description_download_failed",
                    },
                    "labels": {"component": "gemini_client", "severity": "medium"},
                },
            )
            self.langfuse.update_current_trace(
                output=None,
                metadata={"error": error_msg, "failure_reason": "download_failed"},
            )
            return None

        # Create the prompt
        location_context = (
            f" at {location_name}" if location_name else " at this location"
        )
        analysis_prompt = f"""Analyze this image{location_context} and generate a factual, objective description in Georgian language. This description will be used for AI character insertion, so focus on technical and compositional details.

Guidelines:
1. Write in Georgian language only
2. Provide an objective, factual description without emotional language
3. Describe what is physically present in the image (objects, scenery, structures, people without identifying them)
4. Specify the image type: portrait orientation, landscape orientation, square format
5. Note the image quality: sharp/clear, slightly blurry, very blurry
6. Describe the composition: close-up, mid-range, wide shot, aerial view
7. Identify available space: open areas, crowded scene, background space
8. Mention lighting conditions: bright, dim, natural light, indoor lighting
9. Note if there are people present and approximate positioning
10. Keep it factual and technical (3-4 sentences)

Example formats:
"ლანდშაფტული ფოტო მთების ხედით. წინა პლანზე ბალახოვანი ველია, ფონზე თოვლიანი მწვერვალები. ფოტო მკვეთრია, ბუნებრივი განათებით. უხვი ღია სივრცეა წინა პლანზე."

"პორტრეტული ორიენტაციის ფოტო ქალაქის ქუჩის ხედით. შენობები ორივე მხარეს, ცენტრში ცარიელი ქუჩაა. ფოტო ოდნავ უჯერო, საღამოს განათება. საშუალო დისტანციიდან გადაღებული."

"კვადრატული ფორმატის ფოტო სახლის ინტერიერით. ცენტრში მაგიდაა, მარჯვნივ სავარძელი. მკვეთრი ხარისხის, შიდა განათებით. არ არის ხალხი."
"""

        # Create generation span
        with self.langfuse.start_as_current_generation(
            name="gemini_image_description_generation", model=self.model_id
        ) as gen:
            gen.update(
                input={
                    "prompt": analysis_prompt,
                ...
                metadata={
                    "operation_type": "image_description_generation",
                    "location_context": bool(location_name),
                },
            )

            logger.info(
                "Executing Gemini image description generation",
                extra={
                    "json_fields": {
                        "model": self.model_id,
                        "temperature": 0.7,
                        "has_location_name": location_name is not None,
                        "operation": "gemini_image_description_api_call",
                    },
                    "labels": {"component": "gemini_client", "phase": "generation"},
                },
            )
            ...
            try:
                ...
                if response and response.text:
                    ...
                    # Update generation with comprehensive details
                    gen.update(
                        output={
                    ...
                    logger.info(
                        "Gemini image description generation completed successfully",
                        extra={
                            "json_fields": {
                                "description_length": len(description),
                                "input_tokens": response.usage_metadata.prompt_token_count,
                                "output_tokens": response.usage_metadata.candidates_token_count,
                                "total_tokens": response.usage_metadata.total_token_count,
                                "operation": "gemini_image_description_success",
                            },
                            "labels": {
                                "component": "gemini_client",
                                "phase": "generation",
                    ...
                    self.langfuse.update_current_trace(
                        output={"description": description, "success": True},
                    ...
                    error_msg = "No response from Gemini"
                    logger.error(
                        "Gemini image description failed - empty response",
                        extra={
                            "json_fields": {
                                "error": error_msg,
                                "model": self.model_id,
                                "operation": "gemini_image_description_empty_response",
                            },
                            "labels": {
                                "component": "gemini_client",
                                "severity": "high",
                ...
                error_msg = f"Error generating image description with Gemini: {e}"
                logger.error(
                    "Gemini image description generation failed with exception",
                    extra={
                        "json_fields": {
                            "error": str(e),
                            "error_type": type(e).__name__,
                            "model": self.model_id,
                            "operation": "gemini_image_description_exception",
                        },
                        "labels": {"component": "gemini_client", "severity": "high"},
                    },
                )
