...
class ScrapeNewsCivilClient(ScrapeNewsBaseClient):
    def __init__(
        self,
    ...
    async def scrape_news(self, news_quantity: int = 20) -> Optional[NewsResponse]:
        try:
            ...
            if self.use_scrape_do:
                logger.debug(
                    f"Using scrape.do for Civil RSS feed. Target: {self.target_base_url}/{endpoint}"
                )
                ...
                logger.debug(
                    f"Using direct connection for Civil RSS feed. Endpoint: {endpoint}"
                )
                response = await self._make_http_request(endpoint)
                raw_rss_response = await self._handle_xml_response(
                    response, RawGenericRSSResponse
            ...
            for item in items_to_process:
                try:
                    ...
                    logger.warning(
                        f"Could not process Civil RSS item: {item.title}, error: {e}"
                    )
                    continue
            ...
            logger.error(
                f"HTTP error scraping Civil news: {str(e)[:100]}", exc_info=True
            )
            return NewsResponse(news_items=[])
        except Exception as e:
            logger.error(f"Error scraping Civil news: {str(e)[:100]}", exc_info=True)
            return NewsResponse(news_items=[])


@asynccontextmanager
async def get_scrape_civil_news_client():
    client = None
    try:
        client_config = {
            "headers": {
        ...
        if settings.env == "prod":
            logger.debug(
                "Production: Configuring Civil client via Scrape.do with HTTP/1.1"
            )
            client = httpx.AsyncClient(
                base_url=settings.scrape_do_base_url, **client_config
            )
            yield ScrapeNewsCivilClient(
                client,
                use_scrape_do=True,
                target_base_url=settings.scrapable_civil_news_endpoint,
            )
        else:
            logger.debug("Development: Configuring direct Civil client with HTTP/1.1")
            client = httpx.AsyncClient(
                base_url=settings.scrapable_civil_news_endpoint, **client_config
            )
            yield ScrapeNewsCivilClient(client, use_scrape_do=False)
    finally:
        if client:
            logger.debug("Closing Civil client transport")
            await client.aclose()
        else:
            logger.debug("No Civil client was created.")


async def get_scrape_civil_news_dependency():
    async with get_scrape_civil_news_client() as client:
        yield client
